"""
test-general.py - a general test file for KMS, testing
for basic functions and dunder method workability.

Test file version: t-kms-v2026.5.0-scratchattach
Compatible versions for this test file: >=kms-v1.2/2026.1.4 (unpackaged)
"""

VERSION = "t-kms-v2026.5.0-scratchattach"
...
try:
    ...
    raise AssertionError("Key comparison most likely failed, otherwise an unknown error occurred. Check the __sub__ and __rsub__ methods.")
...
assert ((repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk6='val2', sk4='val1', sk5='val3', sk3='val3'])") or
       ...
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk4='val1', sk3='val3', sk5='val3', sk6='val2'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk6='val2', sk3='val3', sk5='val3', sk4='val1'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk5='val3', sk4='val1', sk6='val2', sk3='val3'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk5='val3', sk3='val3', sk4='val1', sk6='val2'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk4='val1', sk5='val3', sk6='val2', sk3='val3'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk3='val3', sk6='val2', sk5='val3', sk4='val1'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk3='val3', sk5='val3', sk4='val1', sk6='val2'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk6='val2', sk5='val3', sk4='val1', sk3='val3'])") or
       (repr(s3|s2) == "Storage(top_lv_key=test1, key_value_pairs=[sk4='val1', sk6='val2', sk3='val3', sk5='val3'])")
...
s1+={'sk5': "will be del"}
assert repr(s1)==("Storage(top_lv_key=test1, key_value_pairs="+
                  "[sk1='hehe', sk2='val2', sk5='will be del'])"
                 )
del s1['sk5']
assert repr(s1)=="Storage(top_lv_key=test1, key_value_pairs=[sk1='hehe', sk2='val2'])"
del s3['sk3']
assert repr(s3)=="Storage(top_lv_key=test2, key_value_pairs=[sk4='val1', sk6='val2'])"
...
with s2 as f:
    assert f == {'sk4': 'val1', 'sk6': 'val2', 'sk3': 'val3'}
...
for item in s2:
    if i==0:
        ...
        assert (
            (set(list(item.keys())).issubset(set(list(s2.values.keys())))) and #type: ignore
            (set(list(item.values())).issubset(set(list(s2.values.values())))) # type: ignore
        )
    i+=1
